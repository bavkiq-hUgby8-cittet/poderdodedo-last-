...
async function handleCardEffect(card, gameData){
  ...
  switch(card.rank){
    ...
    case '7':
      if(rules.length > 0) {
        const removedRules = [...rules]; // Guarda as regras para o log
        rules = []; // Limpa todas as regras
        logs.push(`✂️ ${currName} quebrou TODAS as regras!`);
        
        // Opcional: lista todas as regras quebradas
        removedRules.forEach(rule => {
          logs.push(`   📜 Regra removida: "${rule}"`);
        });
      } else {
